...
// uvVersion is the uv release tamp installs Pythons with.
//
// It is pinned, and the checksums below are pinned with it: bumping uv means
// changing all three lines together, deliberately, with the end-to-end suite
// green. This is also why tamp never runs uv's shell installer — a pipe from
// the network cannot be checked against anything.
const uvVersion = "0.12.6"

// uvChecksums are the SHA-256 digests of the pinned release's Linux tarballs,
// as published alongside them. The key is uv's own target triple.
//
// Only Linux targets appear because only the container's architecture matters:
// tamp downloads on the host but the binary runs inside the bench image,
// which is Debian whichever machine tamp itself is on.
...
// releaseBaseURL is where the pinned release is fetched from. fetchUV takes it
// as an argument so a test can serve the tarball locally.
const releaseBaseURL = "https://github.com/astral-sh/uv/releases/download"

// uvDir is the directory holding the pinned uv, inside the shared volume. The
// version is in the path rather than in a marker file beside it, so a tamp
// upgrade that bumps the pin installs alongside the old one instead of having
// to decide whether what is there is stale.
func uvDir() string { return Dir + "/uv/" + uvVersion }

func uvPath() string { return uvDir() + "/uv" }

// uvTarget maps a container's `uname -m` to uv's target triple.
...
// fetchUV downloads the pinned uv release for a container architecture.
...
// fetchRelease downloads one release tarball, refuses it unless it matches the
// digest tamp ships, and returns the uv binary from inside it.
//
// The refusal is the whole function; the rest is unpacking. A tarball that
// does not match is not unpacked at all, which is the difference between
// pinning a download and merely naming one.
...
// extractUV pulls the one file tamp wants out of the release tarball, which
// also carries uvx.
